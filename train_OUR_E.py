...
parser.add_argument('--lr', type=float, default=1e-3,
                    help='learning rate')
parser.add_argument('--lr_sh_rate', type=int, default=70000,
...
if  args.cur_task == 0:
    avg_mem =int( args.mem_size) # total memory / num of past scenarios
else:
...
model_d = social_stgcnn(n_stgcnn =args.n_stgcnn,n_txpcnn=args.n_txpcnn,
output_feat=args.output_size,seq_len=args.pred_seq_len,
kernel_size=args.kernel_size,pred_seq_len=args.pred_seq_len).cuda()

model_e = social_stgcnn(n_stgcnn =args.n_stgcnn,n_txpcnn=args.n_txpcnn,
output_feat=args.output_size,seq_len=args.obs_seq_len,
kernel_size=args.kernel_size,pred_seq_len=args.pred_seq_len).cuda()

# model_d.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-ODMA/fde_best_3500.pth'))
# model_d.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-ODFTE/fde_best_3500.pth'))
# model_d.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-ODZS/fde_best_1750.pth'))
# model_d.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-ODEP/fde_best_1166.pth'))
model_d.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-ODSR/fde_best_875.pth'))

#Continual learning, load the last learned model
#cur_task: 0, 1, 2, 3, 4
#loaded the last learned model (CL)
if args.cur_task == 1:
    # model_e.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-OEMA/fde_best_3500.pth'))
    print("last model(MA) is loaded")
elif args.cur_task == 2:
    # model_e.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-OEFT/fde_best_3500.pth'))
    print("last model(FT) is loaded")
elif args.cur_task == 3:
    # model_e.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-OEZS/fde_best_{:.0f}.pth'.format(int(args.mem_size/(args.cur_task-1)))))
    print("last model(ZS) is loaded")
elif args.cur_task == 4:
    # model_e.load_state_dict(torch.load(BASE + 'checkpoint/social-stgcnn-GEP/fde_best_{:.0f}.pth'.format(int(args.mem_size/(args.cur_task-1)))))
    print("last model(EP) is loaded")

#Training settings 

# optimizer = optim.SGD(model_e.parameters(),lr=args.lr)
optimizer = optim.AdamW(model_e.parameters(),lr=args.lr)
...
grad_data_num = []
ct_params = 1
for params in model_e.parameters():
    if ct_params in ct_nums:
        continue
...
def train(epoch):
    ...
    if args.cur_task > 0:
        ...
        for tid in range(1, args.tasks):
            ...
            temp = torch.load(BASE + 'mem_data/task' + str(tid) + '.pt')
            temp = temp[:avg_mem]
            memory_data[tid-1].append(temp)
        #print("memory data loaded.")

        # print(memory_data[0][0][0].type)
        for mem_task in range(0, args.cur_task):
            for mem_batch in range(0, avg_mem):
                ...
                # print(tensor.type for tensor in memory_data[mem_task][mem_batch])
                memory_data[mem_task][0][mem_batch] = [tensor.cuda() for tensor in memory_data[mem_task][0][mem_batch]]
    if epoch==0:
        time_record = []
                ...
                for mem_batch in range(0, avg_mem):
                    ...
                    if mem_batch == (avg_mem-1):
                        ...
                        if args.clip_grad is not None:
                            torch.nn.utils.clip_grad_norm_(model_e.parameters(),args.clip_grad)
...
def vald(epoch):
    ...
    ade, fde, _ = val_e(loader_val, model_e, model_d, 1)
    ...
    if  ade < constant_metrics['min_ade_loss']:
        ...
        torch.save(model_e.state_dict(),checkpoint_dir+'ade_best_{:.0f}.pth'.format(avg_mem))  # OK

    if  fde < constant_metrics['min_fde_loss']:
        constant_metrics['min_fde_loss'] =  fde
        constant_metrics['min_fde_epoch'] = epoch
        torch.save(model_e.state_dict(),checkpoint_dir+'fde_best_{:.0f}.pth'.format(avg_mem))  # O


    if  metrics['val_loss'][-1]< constant_metrics['min_val_loss']:
        constant_metrics['min_val_loss'] =  metrics['val_loss'][-1]
        constant_metrics['min_val_epoch'] = epoch
        torch.save(model_e.state_dict(),checkpoint_dir+'val_best_{:.0f}.pth'.format(avg_mem))  # OK

    if epoch%20 == 19:
        torch.save(model_e.state_dict(),checkpoint_dir+'epoch'+str(epoch)+'_best_{:.0f}.pth'.format(avg_mem))  # OK
        torch.save(model_e.state_dict(),checkpoint_dir+'val_best_{:.0f}.pth'.format(avg_mem))  # OK
